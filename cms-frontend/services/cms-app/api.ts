...
export async function cardInquiry(relationshipNum: string) {
  const res = await apiPost<CardInquiryResponse>('/card/inquiry', { relationshipNum });
...
export async function generatePin(payload: {
  ...
  const res = await apiPost<null>('/card/generate-pin', {
    ...payload,
    pin: await encryptPin(payload.pin),
    confirmPin: await encryptPin(payload.confirmPin),
  });
  unwrap(res);
}
...
export async function changePin(payload: {
  ...
  const res = await apiPost<null>('/card/change-pin', {
    ...payload,
    oldPin: await encryptPin(payload.oldPin),
    newPin: await encryptPin(payload.newPin),
    confirmNewPin: await encryptPin(payload.confirmNewPin),
  });
  unwrap(res);
}

export async function availableLimit(pan: string, channelCode: number, tranCode = 1) {
...
export async function authorizeTransaction(payload: {
  ...
  const res = await apiPost<CardAuthorizeResponse>('/card/transaction/authorize', {
    ...payload,
    pin: await encryptPin(payload.pin),
  });
  if (res.responseCode === 10) {
...
export async function validateCard(payload: { pan: string; pin?: string; cvv?: string; track2?: string }) {
  const body = {
    ...payload,
    pin: payload.pin ? await encryptPin(payload.pin) : payload.pin,
  };
  const res = await apiPost<null>('/card/validate', body);
  unwrap(res);
}
...
export async function spendingSummary(payload: {
  ...
  const res = await apiPost<CardSpendingSummaryResponse[]>('/card/spending-summary', payload);
  if (res.responseCode === 3) return [];
  return unwrap(res) || [];
}

export function lovMap(lovs: CardLovResponse[], type: string): Record<string, string> {
