...
import { getLovs, lovMap, newCardRequest } from '@/services/cms-app/api';
import { getSession } from '@/services/cms-app/session';
import { Banner, Field, PageShell, PrimaryButton } from './ui';
...
export function RequestScreen() {
  ...
  const [relationshipNumber, setRelationshipNumber] = useState(session?.relationshipNum || '');
    ...
    <PageShell title="Request card" subtitle="New card application" back>
      ...
      <form className="panel form-stack" onSubmit={onSubmit}>
        ...
        <Field label="Card title">
          <input value={cardTitle} onChange={(e) => setCardTitle(e.target.value)} required placeholder="Cardholder name" />
        </Field>
        ...
        <Field label="Relationship number (13)">
          <input
            ...
            onChange={(e) => setRelationshipNumber(e.target.value.replace(/\D/g, '').slice(0, 13))}
            required
          />
        </Field>
        <Field label="Product">
        ...
        <Field label="Request type">
          <select value={requestTypeId} onChange={(e) => setRequestTypeId(e.target.value)} required>
