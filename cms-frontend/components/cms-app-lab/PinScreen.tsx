...
import { changePin, generatePin, validateCard } from '@/services/cms-app/api';
import {
  getCardContext,
  getSession,
  markPinSet,
...
import { Banner, Field, PageShell, PrimaryButton, PanField } from './ui';
...
export function PinScreen() {
  ...
  const [mode, setMode] = useState<Mode>(() => (readPinAlreadySet() ? 'change' : 'set'));
  ...
  async function onSubmit(e: FormEvent) {
    ...
    try {
      ...
      setPinAlreadySet(true);
      if (mode === 'set') setMode('change');
    } catch (err) {
      const message = err instanceof Error ? err.message : 'PIN update failed';
      if (message.toLowerCase().includes('already set') || message.toLowerCase().includes('pin already')) {
        markPinSet(pan.trim());
        setPinAlreadySet(true);
        setMode('change');
      }
    ...
    <PageShell title="PIN management" subtitle="Set once, then change or reset" back>
      ...
      <form className="panel form-stack" onSubmit={onSubmit}>
        ...
        <PrimaryButton type="submit" loading={loading}>
          {mode === 'change' ? 'Change PIN' : mode === 'forgot' ? 'Reset PIN' : 'Set PIN'}
        </PrimaryButton>
      </form>
      ...
      <form
        ...
        onSubmit={async (e) => {
          ...
          try {
            await validateCard({ pan: pan.trim(), pin: checkPin || undefined, cvv: checkCvv || undefined });
      ...
      >
        <h3>Validate card</h3>
        <p className="hint">Calls /card/validate (PIN/CVV). PIN is AES-encrypted.</p>
        <div className="form-grid">
          ...
          <Field label="CVV (optional)">
            <input
              ...
              maxLength={4}
              value={checkCvv}
              onChange={(e) => setCheckCvv(e.target.value.replace(/\D/g, ''))}
            />
          </Field>
        </div>
        <PrimaryButton type="submit" loading={checkLoading}>
          Validate
        </PrimaryButton>
      </form>
    </PageShell>
