...
import { availableLimit, spendingSummary } from '../api/cms';
import { Banner, Field, PageShell, PrimaryButton, SecondaryButton, PanField } from '../components/ui';
import {
  formatMoney,
  getCardContext,
  getCustomLimit,
...
import type { CardLimitValidateResponse } from '../types/cms';
...
type ChannelLimit = {
  code: number;
  label: string;
  /** Profile / server max ceiling */
  profileMax: number | null;
  /** Remaining from server (against profile) */
...
function resolveDisplay(
  ...
  const spent =
    profileMax != null && serverAvailable != null
      ? Math.max(0, profileMax - serverAvailable)
      : 0;
  const stored = pan ? getCustomLimit(pan, channelCode) : null;
  const customized =
    stored != null && profileMax != null
      ? Math.min(Math.max(0, stored), profileMax)
      : profileMax;
  const available =
    customized != null ? Math.max(0, customized - spent) : serverAvailable;
  const usedPct =
    customized != null && customized > 0
      ? Math.min(100, Math.round((spent / customized) * 100))
      : 0;
...
export function LimitsScreen() {
  ...
  const loadDashboard = useCallback(async () => {
    ...
    try {
      const rows = await Promise.all(
        CHANNELS.map((ch) => loadChannelLimit(panValue, acct, ch)),
      );
      setChannels(rows);
      ...
      for (const row of rows) {
        ...
        nextDrafts[row.code] =
          display.customized != null ? String(display.customized) : '';
      }
  ...
  function saveChannelCustom(code: number, profileMax: number | null) {
    ...
    setSuccess(
      `${CHANNELS.find((c) => c.code === code)?.label || 'Channel'} customized limit saved (on this device).`,
    );
    setTick((t) => t + 1);
    setSavingCode(null);
  }
  ...
  return (
    <PageShell
      ...
      subtitle="Available daily · customize under max"
      back
    >
              ...
              {hasData ? (
                <>
                  ...
                  <p className="limits-cap">
                    ...
                    {profileMax != null && display.customized !== profileMax ? (
                      <> · max {formatMoney(profileMax)}</>
                    ) : null}
                  </p>
                  ...
                  <div className="limits-bar" aria-hidden>
                    <div className="limits-bar-track">
                      <div
                        ...
                        style={{
                          width: `${Math.max(display.usedPct, display.usedPct > 0 ? 4 : 0)}%`,
                        }}
                      />
                    </div>
                    ...
                    <div className="limits-edit">
                      ...
                      <div className="limits-edit-row">
                        <input
                          ...
                          onChange={(e) =>
                            onDraftChange(ch.code, e.target.value, profileMax)
                          }
                        />
                        <input
                          type="number"
                      ...
                      <PrimaryButton
                        type="button"
                        loading={savingCode === ch.code}
                        onClick={() => saveChannelCustom(ch.code, profileMax)}
                      >
                        Save {ch.label} limit
                      </PrimaryButton>
                    </div>
